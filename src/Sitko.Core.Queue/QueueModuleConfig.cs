...
{
    [JsonIgnore] public HashSet<Type> Middlewares { get; } = new();
    ...
    public void ConfigureMessage<T>(IQueueMessageOptions<T> options) where T : class
    {
        ...
        {
            throw new InvalidOperationException($"Options for type {typeof(T)} already registered");
        }
    ...
    public void RegisterProcessors<TAssembly>()
    {
        ...
        foreach (var type in assembly.ExportedTypes)
        {
            ...
            if (method == null)
            {
                throw new InvalidOperationException("Can't find method RegisterProcessor");
            }
            ...
            foreach (var implementedInterface in type.GetInterfaces())
            {
                    ...
                    implementedInterface.IsGenericType)
                {
                    if (!type.IsPublic)
                    {
                        throw new InvalidOperationException($"Processor class {type.FullName} must be public");
                    }
